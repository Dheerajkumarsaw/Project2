...
const createIntern = async function (req, res) {
    try {
        ...
        if (unique) return res.status(401).send({ status: false, message: "Use Different Email or Password" }); ////change status code
...
const collegeDetails = async function (req, res) {
    try {
        ...
        const college = await collegeModel.findOne({ name: collegeName })
        if (!college) return res.status(404).send({ status: false, message: 'College Not Found, Register First' });
        ...
        if (!internList) return res.status(404).send({ status: false, message: `${collegeName} Not Have Any Internship` });
